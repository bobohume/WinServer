CREATE PROCEDURE [dbo].[Sp_Mall_Off_Goods_Shelf_Spc] 
...
BEGIN
	...
	BEGIN TRANSACTION Mall_Off_Goods_Shelf_Spc
	...
	IF @ERR = 0
	BEGIN
		IF NOT EXISTS (SELECT 1 FROM Tbl_Price WHERE GoodsID IN (SELECT GoodsID FROM Tbl_Goods WHERE GoodsName LIKE @GoodsName)
		...
		BEGIN
			...
			UPDATE Tbl_Price SET EndTime = '2012-01-01 00:00:00'
				WHERE GoodsID IN (SELECT GoodsID FROM Tbl_Goods WHERE GoodsName LIKE @GoodsName)
					AND PriceID IN (SELECT PriceID FROM Tbl_PriceTaxon WHERE PriceRackID = @PriceRackID)
	...
	IF @ERR = 0
		COMMIT TRANSACTION Mall_Off_Goods_Shelf_Spc
	ELSE
		ROLLBACK TRANSACTION Mall_Off_Goods_Shelf_Spc
