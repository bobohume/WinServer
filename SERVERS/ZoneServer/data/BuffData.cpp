...
bool CBuffDataRes::read()
{
	...
	for (int i=0; i<file.RecordNum; i++) {
		...
		file.GetData(LineData);
		ASSERT(LineData.m_Type==DType_S32, "read buffdata 状态标识 error");
		pdata->m_Flags=LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type==DType_F64, "read buffdata 组 error");
		pdata->m_Group=LineData.m_F64;

		file.GetData(LineData);
		ASSERT(LineData.m_Type==DType_F64, "read buffdata 移除组 error");
		pdata->m_RemoveGroup=LineData.m_F64;

		file.GetData(LineData);
		ASSERT(LineData.m_Type==DType_F64, "read buffdata 免疫组error");
		pdata->m_ImmunitGroup=LineData.m_F64;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 视野 error");
		pdata->m_Stats.See = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 生命 error");
		pdata->m_Stats.HP = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 生命百分比 error");
		pdata->m_Stats.HP_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 最大生命 error");
		pdata->m_Stats.MaxHP = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 最大生命百分比 error");
		pdata->m_Stats.MaxHP_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 每回合回血 error");
		pdata->m_Stats.RecoverHP = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 最大生命每回合回血百分比 error");
		pdata->m_Stats.RecoverHp_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 攻击吸血 error");
		pdata->m_Stats.DeriveHP = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 体力 error");
		pdata->m_Stats.Str = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 体力百分比 error");
		pdata->m_Stats.Str_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 最大体力 error");
		pdata->m_Stats.MaxStr = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 最大体力百分比 error");
		pdata->m_Stats.MaxStr_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 每回合消耗体力 error");
		pdata->m_Stats.ReduceStr = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 爆击几率 error");
		pdata->m_Stats.Cri_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 普攻闪避几率 error");
		pdata->m_Stats.Doe_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 远程类武器闪避几率 error");
		pdata->m_Stats.DoeR_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 速度 error");
		pdata->m_Stats.Spe = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 物理攻击 error");
		pdata->m_Stats.PhyDamage = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 物理攻击百分比 error");
		pdata->m_Stats.PhyDamage_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 水攻击 error");
		pdata->m_Stats.ShuiDamage = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 水攻击百分比 error");
		pdata->m_Stats.ShuiDamage_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 火攻击error");
		pdata->m_Stats.HuoDamage = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 火攻击百分比 error");
		pdata->m_Stats.HuoDamage_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 电攻击error");
		pdata->m_Stats.JinDamage = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 电攻击百分比 error");
		pdata->m_Stats.JinDamage_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 物理防御 error");
		pdata->m_Stats.PhyDefence = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 物理防御百分比 error");
		pdata->m_Stats.PhyDefence_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 物理伤害减免error");
		pdata->m_Stats.PhyImmunit = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 物理伤害减免百分比 error");
		pdata->m_Stats.PhyImmunit_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 水伤害减免error");
		pdata->m_Stats.ShuiDefence = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 水伤害减免百分比 error");
		pdata->m_Stats.ShuiDefence_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 火伤害减免error");
		pdata->m_Stats.HuoDefence = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 火伤害减免百分比 error");
		pdata->m_Stats.HuoDefence_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 电伤害减免error");
		pdata->m_Stats.JinDefence = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 电伤害减免百分比 error");
		pdata->m_Stats.JinDefence_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 对怪物伤害增加50% error");
		readFamily(pdata->m_Stats, LineData.m_F32, true);

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 减少怪物伤害50% error");
		readFamily(pdata->m_Stats, LineData.m_F32, false);

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_S32, "read buffdata 远程武器攻击error");
		pdata->m_Stats.RemoteAtk = LineData.m_S32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 远程武器伤害修正百分比 error");
		pdata->m_Stats.RemoteAtk_gPc = LineData.m_F32;

		file.GetData(LineData);
		ASSERT(LineData.m_Type == DType_F32, "read buffdata 反击伤害百分比 error");
		pdata->m_Stats.RemoteAtk_gPc = LineData.m_F32;
