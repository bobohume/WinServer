...
bool CItemDataRes::read()
{
	...
	if(!file.readDataFile("data/Data_1_Item.dat"))
	{
	...
	for(int i = 0; i < file.RecordNum; ++i)
	{
		...
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata ID error");
		...
		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_string, "read itemdata icon error");
		pdata->m_sIcon= LineData.m_string;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_enum16, "read itemdata type error");
		...
		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata limit error");
		pdata->m_Limit = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata Mpro error");
		pdata->m_Mpro = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata attwidth error");
		pdata->m_AttWidth = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata attlength error");
		pdata->m_AttLength = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata buy error");
		pdata->m_Buy = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata sale error");
		pdata->m_Sale = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata skill error");
		pdata->m_Skill = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata timelimit error");
		pdata->m_Timelimit = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata usetimes error");
		pdata->m_UseTimes = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata Maxdie error");
		pdata->m_MaxDie = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata cD error");
		pdata->m_CD = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata buffID1 error");
		pdata->m_BuffID1 = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata buffID2 error");
		pdata->m_BuffID2 = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata trap error");
		pdata->m_Trap = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata effic error");
		pdata->m_Effic = LineData.m_S32;

		file.GetData(LineData);
		DOASSERT(LineData.m_Type == DType_S32, "read itemdata ±¦Ê¯¿×ÊýÁ¿ error");
