...
#define REFID_FUNCTION_MAPPING "svrfix_refid_function_mapping"
...
void lua_tinker::init(lua_State *L)
{
	...
	lua_pushstring(L, REFID_FUNCTION_MAPPING);
    lua_newtable(L);
...
{
	char temp[64];
	sprintf_s(temp,sizeof(temp),"%I64d", *(long long*)lua_topointer(L, 1));
...
{
	char temp[64];
	sprintf_s(temp,sizeof(temp),"%I64u", *(unsigned long long*)lua_topointer(L, 1));
	lua_pushstring(L, temp);
	return 1;
}
...
{
	const char* ptr = lua_tostring(L, index);				
	return ptr ? ptr : "";
}
...
template<>
void lua_tinker::push(lua_State *L, table* ret)
{
	lua_pushvalue(L, ret->m_obj->m_index);
}
...
int lua_tinker::ref_function(lua_State* L, int lo, int def)
{
	// function at lo
...
void lua_tinker::get_function_by_refid(lua_State* L, int refid)
{
    lua_pushstring(L, REFID_FUNCTION_MAPPING);
...
{
    lua_tinker::get_function_by_refid(L, nHandler);                  /* L: ... func */
...
void lua_tinker::remove_function_by_refid(lua_State* L, int refid)
{
    lua_pushstring(L, REFID_FUNCTION_MAPPING);
    lua_rawget(L, LUA_REGISTRYINDEX);                           /* stack: ... refid_fun */
    lua_pushinteger(L, refid);                                  /* stack: ... refid_fun refid */
...
int executeFunction(lua_State* L,int numArgs)
{
    ...
    else
    {
        ...
        errfunc = functionIndex - 1;
    }

    int error = 0;
    error = lua_pcall(L, numArgs, 1, errfunc);                  /* L: ... ret */
...
void lua_tinker::executeFunctionByHandler(lua_State* L,int nHandler,int numArgs)
{
