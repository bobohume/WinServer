...
/*********************************************************************************************
 ...
 1:初始化函数,调用LUA初始化函数，创建lua state,并打开基础库...
 ...
 3:导出全局函数,其中函数的参数必须<=6个参数.如果超过请用table来代替参数，包括后面任何参数超过的情况。参数类型和返回值支持模板
 ...
 4:调用脚本全局函数
   ...
   int result = lua_tinker::call<int>(L, "lua_func",haha);

   或

   int result = lua_tinker::call<int>(L, "lua_func", 3, 4);

   其中lua_func表示lua脚本，后面可以跟普通参数或table.
 ...
 6:导出全局变量值到脚本.其中脚本导出后，值不会随着程序的改变而改变，需要重新设置
   static int cpp_int = 1;
   lua_tinker::set(L, "cpp_int", cpp_int);

 7:获取脚本变量,模板指定返回的类型
   int lua_int = lua_tinker::get<int>(L, "lua_int");

 8:导出类，类函数，成员。其中导出的成员变量会随脚本改变而改变.反之亦然
   struct base
   {
	  base() {}
   ...
   lua_tinker::class_mem<base>(L, "m",		 &base::m);
 ...
 9:继承.
   ...
   public:
	 ...
	 const char* is_child(){ return "this is child"; }
   };

   先导出子类.然后再声明继承关系
   lua_tinker::class_add<child>(L, "child");
   lua_tinker::class_inh<child,base>(L);
   lua_tinker::class_def<child>(L, "is_child", &child::is_child);

 10:脚本内对象创建
    首先需要定义构造函数，在脚本里面,参数<=5,不够用table
	lua_tinker::class_con<child>(L, lua_tinker::constructor<child,int>);

	这样脚本里面就可以通过temp = child(4);来创建对象

	程序中创建的对象,可以通过设置到全局变量表里面
	child g_child;
	lua_tinker::set(L, "g_child", &g_child);

	脚本里面就可以通过 g_child:is_child来访问方法g_child.m来访问成员

 11:TABLE创建
    lua_tinker::table haha(L, "haha");

 12:错误处理,定义导出1个_ALERT函数,来输出脚本中的错误。
...
namespace lua_tinker
{
	...
	// functor
	template<typename RVal, typename T1 = void, typename T2 = void, typename T3 = void, typename T4 = void, typename T5 = void, typename T6 = void, typename T7 = void, typename T8 = void, typename T9 = void, typename T10 = void, typename T11 = void>
	struct functor
	{
		static int invoke(lua_State *L) { push(L, upvalue_<RVal(*)(T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11)>(L)(read<T1>(L, 1), read<T2>(L, 2), read<T3>(L, 3), read<T4>(L, 4), read<T5>(L, 5), read<T6>(L, 6), read<T7>(L, 7), read<T8>(L, 8), read<T9>(L, 9), read<T10>(L, 10), read<T11>(L, 11))); return 1; }
	};
	...
	{
		static int invoke(lua_State *L) { upvalue_<void(*)(T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11)>(L)(read<T1>(L, 1), read<T2>(L, 2), read<T3>(L, 3), read<T4>(L, 4), read<T5>(L, 5), read<T6>(L, 6), read<T7>(L, 7), read<T8>(L, 8), read<T9>(L, 9), read<T10>(L, 10), read<T11>(L, 11)); return 0; }
	};
	...
	template<typename T>
	struct mem_var_offset : var_base
	{
		U8  type;
		U16 offset;
		
		mem_var_offset(U16 _offset,U8 _type) : offset(_offset),type(_type) {}
		...
		{ 
			...
			void* pBase = (void *) (((const char *)(ptr)) + offset);

			switch(type)
			{
			case DType_string:
				{
					push(L,*((const char**)pBase));
				}
				break;
			case DType_enum8:
			case DType_enum16:
	...
	// member function
	template<typename RVal, typename T, typename T1 = void, typename T2 = void, typename T3 = void, typename T4 = void, typename T5 = void, typename T6 = void, typename T7 = void, typename T8 = void, typename T9 = void, typename T10 = void, typename T11 = void>
	struct mem_functor
	{
		static int invoke(lua_State *L) { push(L, (read<T*>(L, 1)->*upvalue_<RVal(T::*)(T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11)>(L))(read<T1>(L, 2), read<T2>(L, 3), read<T3>(L, 4), read<T4>(L, 5), read<T5>(L, 6), read<T6>(L, 7), read<T7>(L, 8), read<T8>(L, 9), read<T9>(L, 10), read<T10>(L, 11), read<T11>(L, 12))); return 1; }
	};
	...
	template<typename RVal, typename T, typename T1, typename T2, typename T3, typename T4, typename T5, typename T6, typename T7, typename T8, typename T9, typename T10, typename T11>
	...
	void class_mem(lua_State* L, const char* memname, VAR BASE::*val) 
	{ 
		push_meta(L, class_name<T>::name());
		if(lua_istable(L, -1))
		{
			lua_pushstring(L, memname);
	...
	void class_mem_offset(lua_State* L, const char* memname, U16 offset,U8 type) 
	{ 
		push_meta(L, class_name<T>::name());
		...
		{
			lua_pushstring(L, memname);
			new(lua_newuserdata(L,sizeof(mem_var_offset<BASE>))) mem_var_offset<BASE>(offset,type);
			lua_rawset(L, -3);
		}
	...
	int  ref_function(lua_State* L, int lo, int def);
	void get_function_by_refid(lua_State* L, int refid);
	void remove_function_by_refid(lua_State* L, int refid);
	void executeFunctionByHandler(lua_State* L,int nHandler,int numArgs);
