...
{
   LIB_ASSERT(p != presult, "Error, aliasing matrix mul pointers not allowed here!");
...
{
   LIB_ASSERT(v != vresult, "Error, aliasing matrix mul pointers not allowed here!");
...
inline U32 mClamp(U32 val, U32 low, U32 high)
{
	return getMax(getMin(val, high), low);
}
...
inline U64 mClamp(U64 val, U64 low, U64 high)
{
	return getMax(getMin(val, high), low);
}
