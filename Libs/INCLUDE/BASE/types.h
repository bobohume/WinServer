...
static const S64 S64_MIN = S64(-0x7fffffffffffffff - 1);          ///< Constant Min Limit S64
static const S64 S64_MAX = S64(0x7fffffffffffffff);               ///< Constant Max Limit S64
static const U64 U64_MAX = U64(0xffffffffffffffff);               ///< Constant Max Limit U64

static const F32 GAMEPLAY_PERCENT_SCALE = 100.0f;
//--------------------------------------
...
#define BIT64(x) (((U64)1) << (x))                       ///< Returns value with bit x set (2^x)
...
{
	return new(p) T;  //callback the object Construct Function
}
...
#define Variable_To_Str(x)  (#x)

inline char *dStrncpy(char *dst, size_t dstSize, const char *src, size_t len,bool term = false)
{
	if(!dst)
		return NULL;
...
inline char *dStrcpy(char *dst, size_t dstSize, const char *src)
{
	return dStrncpy(dst, dstSize,src,dStrlen(src),true);
}
...
inline void backslash( wchar_t* str )
{
	backslashT< wchar_t >( str );
}
